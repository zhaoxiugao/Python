...
def _fulltest_():
    ...
    k = triple_des("h1k2#3s4f5d6%7d8s9@0s1f2", mode=CBC, IV="20151008")
    data = "new Ba String".encode('ascii')
    d = k.encrypt(data, padmode=PAD_PKCS5)
    if k.decrypt(d, padmode=PAD_PKCS5) != data:
    ...
    k = triple_des("h1k2#3s4f5d6%7d8s9@0s1f2", mode=CBC, IV="20151008", padmode=PAD_PKCS5)
    data = "new Ba String".encode('ascii')
    d = k.encrypt(data)
    if k.decrypt(d) != data:
        print d
    ...
    k = triple_des("h1k2#3s4f5d6%7d8s9@0s1f2", mode=CBC, pad=None, padmode=PAD_PKCS5)
    data = "new Ba String".encode('ascii')
    d = k.encrypt(data)
    if k.decrypt(d) != data:
